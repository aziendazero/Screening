package model.agenda;

import model.agenda.common.Agenda_SoAppuntamentoViewRow;

import oracle.jbo.RowIterator;
import oracle.jbo.domain.Date;
import oracle.jbo.server.AttributeDefImpl;
import oracle.jbo.server.ViewRowImpl;
// ---------------------------------------------------------------------
// ---    File generated by Oracle ADF Business Components Design Time.
// ---    Fri Nov 09 13:28:27 CET 2018
// ---    Custom code may be added to this class.
// ---    Warning: Do not modify method signatures of generated methods.
// ---------------------------------------------------------------------
public class Agenda_SoAppuntamentoViewRowImpl extends ViewRowImpl implements Agenda_SoAppuntamentoViewRow {


    public static final int ENTITY_AGENDA_SOAPPUNTAMENTO = 0;

    /**
     * AttributesEnum: generated enum for identifying attributes and accessors. DO NOT MODIFY.
     */
    public enum AttributesEnum {
        Idapp {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getIdapp();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setIdapp((Integer) value);
            }
        }
        ,
        Idcentro {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getIdcentro();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setIdcentro((Integer) value);
            }
        }
        ,
        Dtapp {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getDtapp();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setDtapp((Date) value);
            }
        }
        ,
        Oraapp {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getOraapp();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setOraapp((Integer) value);
            }
        }
        ,
        Minapp {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getMinapp();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setMinapp((Integer) value);
            }
        }
        ,
        Dispslot {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getDispslot();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setDispslot((Integer) value);
            }
        }
        ,
        Tpsrc {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getTpsrc();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setTpsrc((String) value);
            }
        }
        ,
        Orario {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getOrario();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setOrario((String) value);
            }
        }
        ,
        RigaSalvata {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getRigaSalvata();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setRigaSalvata((String) value);
            }
        }
        ,
        Slotoccup {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getSlotoccup();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setSlotoccup((Integer) value);
            }
        }
        ,
        Stato {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getStato();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setStato((String) value);
            }
        }
        ,
        Nascosto {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getNascosto();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setNascosto((Boolean) value);
            }
        }
        ,
        Agenda_DettAppColonView {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getAgenda_DettAppColonView();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setAttributeInternal(index(), value);
            }
        }
        ,
        Agenda_DettAppView {
            public Object get(Agenda_SoAppuntamentoViewRowImpl obj) {
                return obj.getAgenda_DettAppView();
            }

            public void put(Agenda_SoAppuntamentoViewRowImpl obj, Object value) {
                obj.setAttributeInternal(index(), value);
            }
        }
        ;
        static AttributesEnum[] vals = null;
        ;
        private static final int firstIndex = 0;

        public abstract Object get(Agenda_SoAppuntamentoViewRowImpl object);

        public abstract void put(Agenda_SoAppuntamentoViewRowImpl object, Object value);

        public int index() {
            return AttributesEnum.firstIndex() + ordinal();
        }

        public static final int firstIndex() {
            return firstIndex;
        }

        public static int count() {
            return AttributesEnum.firstIndex() + AttributesEnum.staticValues().length;
        }

        public static final AttributesEnum[] staticValues() {
            if (vals == null) {
                vals = AttributesEnum.values();
            }
            return vals;
        }
    }


    public static final int IDAPP = AttributesEnum.Idapp.index();
    public static final int IDCENTRO = AttributesEnum.Idcentro.index();
    public static final int DTAPP = AttributesEnum.Dtapp.index();
    public static final int ORAAPP = AttributesEnum.Oraapp.index();
    public static final int MINAPP = AttributesEnum.Minapp.index();
    public static final int DISPSLOT = AttributesEnum.Dispslot.index();
    public static final int TPSRC = AttributesEnum.Tpsrc.index();
    public static final int ORARIO = AttributesEnum.Orario.index();
    public static final int RIGASALVATA = AttributesEnum.RigaSalvata.index();
    public static final int SLOTOCCUP = AttributesEnum.Slotoccup.index();
    public static final int STATO = AttributesEnum.Stato.index();
    public static final int NASCOSTO = AttributesEnum.Nascosto.index();
    public static final int AGENDA_DETTAPPCOLONVIEW = AttributesEnum.Agenda_DettAppColonView.index();
    public static final int AGENDA_DETTAPPVIEW = AttributesEnum.Agenda_DettAppView.index();

    /**
     * This is the default constructor (do not remove).
     */
    public Agenda_SoAppuntamentoViewRowImpl() {
    }

    /**
     * Gets Agenda_SoAppuntamento entity object.
     * @return the Agenda_SoAppuntamento
     */
    public Agenda_SoAppuntamentoImpl getAgenda_SoAppuntamento() {
        return (Agenda_SoAppuntamentoImpl) getEntity(ENTITY_AGENDA_SOAPPUNTAMENTO);
    }

    /**
     * Gets the attribute value for IDAPP using the alias name Idapp.
     * @return the IDAPP
     */
    public Integer getIdapp() {
        return (Integer) getAttributeInternal(IDAPP);
    }

    /**
     * Sets <code>value</code> as attribute value for IDAPP using the alias name Idapp.
     * @param value value to set the IDAPP
     */
    public void setIdapp(Integer value) {
        setAttributeInternal(IDAPP, value);
    }

    /**
     * Gets the attribute value for IDCENTRO using the alias name Idcentro.
     * @return the IDCENTRO
     */
    public Integer getIdcentro() {
        return (Integer) getAttributeInternal(IDCENTRO);
    }

    /**
     * Sets <code>value</code> as attribute value for IDCENTRO using the alias name Idcentro.
     * @param value value to set the IDCENTRO
     */
    public void setIdcentro(Integer value) {
        setAttributeInternal(IDCENTRO, value);
    }

    /**
     * Gets the attribute value for DTAPP using the alias name Dtapp.
     * @return the DTAPP
     */
    public Date getDtapp() {
        return (Date) getAttributeInternal(DTAPP);
    }

    /**
     * Sets <code>value</code> as attribute value for DTAPP using the alias name Dtapp.
     * @param value value to set the DTAPP
     */
    public void setDtapp(Date value) {
        setAttributeInternal(DTAPP, value);
    }

    /**
     * Gets the attribute value for ORAAPP using the alias name Oraapp.
     * @return the ORAAPP
     */
    public Integer getOraapp() {
        return (Integer) getAttributeInternal(ORAAPP);
    }

    /**
     * Sets <code>value</code> as attribute value for ORAAPP using the alias name Oraapp.
     * @param value value to set the ORAAPP
     */
    public void setOraapp(Integer value) {
        setAttributeInternal(ORAAPP, value);
    }

    /**
     * Gets the attribute value for MINAPP using the alias name Minapp.
     * @return the MINAPP
     */
    public Integer getMinapp() {
        return (Integer) getAttributeInternal(MINAPP);
    }

    /**
     * Sets <code>value</code> as attribute value for MINAPP using the alias name Minapp.
     * @param value value to set the MINAPP
     */
    public void setMinapp(Integer value) {
        setAttributeInternal(MINAPP, value);
    }

    /**
     * Gets the attribute value for DISPSLOT using the alias name Dispslot.
     * @return the DISPSLOT
     */
    public Integer getDispslot() {
        return (Integer) getAttributeInternal(DISPSLOT);
    }

    /**
     * Sets <code>value</code> as attribute value for DISPSLOT using the alias name Dispslot.
     * @param value value to set the DISPSLOT
     */
    public void setDispslot(Integer value) {
        setAttributeInternal(DISPSLOT, value);
    }

    /**
     * Gets the attribute value for TPSRC using the alias name Tpsrc.
     * @return the TPSRC
     */
    public String getTpsrc() {
        return (String) getAttributeInternal(TPSRC);
    }

    /**
     * Sets <code>value</code> as attribute value for TPSRC using the alias name Tpsrc.
     * @param value value to set the TPSRC
     */
    public void setTpsrc(String value) {
        setAttributeInternal(TPSRC, value);
    }

    /**
     * Gets the attribute value for the calculated attribute Orario.
     * @return the Orario
     */
    public String getOrario() {
        return (String) getAttributeInternal(ORARIO);
    }

    /**
     * Sets <code>value</code> as the attribute value for the calculated attribute Orario.
     * @param value value to set the  Orario
     */
    public void setOrario(String value) {
        setAttributeInternal(ORARIO, value);
    }

    /**
     * Gets the attribute value for the calculated attribute RigaSalvata.
     * @return the RigaSalvata
     */
    public String getRigaSalvata() {
        return (String) getAttributeInternal(RIGASALVATA);
    }

    /**
     * Sets <code>value</code> as the attribute value for the calculated attribute RigaSalvata.
     * @param value value to set the  RigaSalvata
     */
    public void setRigaSalvata(String value) {
        setAttributeInternal(RIGASALVATA, value);
    }

    /**
     * Gets the attribute value for SLOTOCCUP using the alias name Slotoccup.
     * @return the SLOTOCCUP
     */
    public Integer getSlotoccup() {
        return (Integer) getAttributeInternal(SLOTOCCUP);
    }

    /**
     * Sets <code>value</code> as attribute value for SLOTOCCUP using the alias name Slotoccup.
     * @param value value to set the SLOTOCCUP
     */
    public void setSlotoccup(Integer value) {
        setAttributeInternal(SLOTOCCUP, value);
    }

    /**
     * Gets the attribute value for the calculated attribute Stato.
     * @return the Stato
     */
    public String getStato() {
        return (String) getAttributeInternal(STATO);
    }

    /**
     * Sets <code>value</code> as the attribute value for the calculated attribute Stato.
     * @param value value to set the  Stato
     */
    public void setStato(String value) {
        setAttributeInternal(STATO, value);
    }


    /**
     * Gets the attribute value for NASCOSTO using the alias name Nascosto.
     * @return the NASCOSTO
     */
    public Boolean getNascosto() {
        return (Boolean) getAttributeInternal(NASCOSTO);
    }

    /**
     * Sets <code>value</code> as attribute value for NASCOSTO using the alias name Nascosto.
     * @param value value to set the NASCOSTO
     */
    public void setNascosto(Boolean value) {
        setAttributeInternal(NASCOSTO, value);
    }

    /**
     * Gets the associated <code>RowIterator</code> using master-detail link Agenda_DettAppColonView.
     */
    public RowIterator getAgenda_DettAppColonView() {
        return (RowIterator) getAttributeInternal(AGENDA_DETTAPPCOLONVIEW);
    }

    /**
     * Gets the associated <code>RowIterator</code> using master-detail link Agenda_DettAppView.
     */
    public RowIterator getAgenda_DettAppView() {
        return (RowIterator) getAttributeInternal(AGENDA_DETTAPPVIEW);
    }

    /**
     * getAttrInvokeAccessor: generated method. Do not modify.
     * @param index the index identifying the attribute
     * @param attrDef the attribute

     * @return the attribute value
     * @throws Exception
     */
    protected Object getAttrInvokeAccessor(int index, AttributeDefImpl attrDef) throws Exception {
        if ((index >= AttributesEnum.firstIndex()) && (index < AttributesEnum.count())) {
            return AttributesEnum.staticValues()[index - AttributesEnum.firstIndex()].get(this);
        }
        return super.getAttrInvokeAccessor(index, attrDef);
    }

    /**
     * setAttrInvokeAccessor: generated method. Do not modify.
     * @param index the index identifying the attribute
     * @param value the value to assign to the attribute
     * @param attrDef the attribute

     * @throws Exception
     */
    protected void setAttrInvokeAccessor(int index, Object value, AttributeDefImpl attrDef) throws Exception {
        if ((index >= AttributesEnum.firstIndex()) && (index < AttributesEnum.count())) {
            AttributesEnum.staticValues()[index - AttributesEnum.firstIndex()].put(this, value);
            return;
        }
        super.setAttrInvokeAccessor(index, value, attrDef);
    }
}

